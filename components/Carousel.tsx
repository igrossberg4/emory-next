...
interface ButtonEnabled {
  ...
  export default function EmblaCarousel ({ slides, actual, navigation }: { slides: any; actual: any, navigation:boolean }){
    const router = useRouter();
    if(!navigation){
        console.log("No nav");
    }
    const index = actual ? slides.findIndex(
    ...
    return (
      ...
      <AnimatePresence>
        <div className="embla__viewport" key={"viewPort"}>
          ...
          <motion.div
            ...
            onTransitionEnd={(e) => {
              ...
              if (target.className === 'header-inner-content' && navigation === true) {
                        ...
                        view={[
                          {
                            ...
                            props: { view: value?.props?.view ? [value.props.view[0]] :[value] },
                          },
                        ]}
                      ></DynamicComponentMatcher>
                    </div>
        ...
        />
        <NextButton
          href={actual ? actual.next : (page === slides.length -1 ? '' : 'active')}
          onClick={scrollNext}
